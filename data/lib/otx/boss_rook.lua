BossRook = {
	...
	action_ids = {
		lever = 12355, -- action id da alavanca
		chest = 12356, -- action id do bau
	},

	death_event = "BossRoookDeath", -- nome do evento de death do creaturescripts
	...
	places = {
		boss_room = {
			-- sala do boss
			destination = {x = 31951, y = 32171, z = 14}, -- posição do destino (onde boss é criado)
			area = {from = {x = 31948, y = 32151, z = 14}, to = {x = 31976, y = 32146, z = 14}} -- area
		},
		reward_room = {
			-- sala de recompensa
...
do
	local str = ""
	for i = 1, #BossRook.items_needed do
		local item = BossRook.items_needed[i]
	...
	BossRook.items_description = (str:sub(1, #str - 2):gsub("(.+), ", "%1 e ", 1))
end

math.randomseed(os.mtime())

function BossRook:genSerial()
	local serial = "S"
	for _ = 1, 11 do
...
function BossRook:getSerial()
	return getStorage(self.serial_storage)
end

function BossRook:getSpectators(place, getPlayers, getMonsters)
...
function BossRook:getRandomBoss()
	local chance = nil
...
function BossRook:createBoss(name)
...
-- CALLBACKS
function BossRook:onUse(cid, item, fromPosition, itemEx, toPosition, isHotkey)
	-- Alavanca
...
function BossRook:onDeath(cid, corpse, deathList)
	local creatures = self:getSpectators("boss_room", true, true)
	for i = 1, #creatures do
