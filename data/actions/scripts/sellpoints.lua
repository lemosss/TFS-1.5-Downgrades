...
function onUse(cid, item, fromPosition, itemEx, toPosition)
	...
	if item.itemid == 8351 then
		if doRemoveItem(item.uid) then
			query("UPDATE `accounts` SET `premium_points` = `premium_points` + 1 WHERE `id` = '"..getPlayerAccountId(cid).."'")
			doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_ORANGE, "[Sell Points System] Você recebeu 1 points!")
