...
if Modules == nil then
	...
	function ShopModule.requestTrade(cid, message, keywords, parameters, node)
		...
		-- Open the modern shop window (since Tibia 8.2). Works for any non-TALK mode.
		-- openShopWindow is a global registered by npc.cpp, available inside NPC scripts.
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
			openShopWindow(cid, itemWindow,
				function(cid, itemid, subType, amount, ignoreCap, inBackpacks)
					module.npcHandler:onBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
				end,
				function(cid, itemid, subType, amount, ignoreEquipped, _)
					module.npcHandler:onSell(cid, itemid, subType, amount, ignoreEquipped, _)
				end)
		end
			...
			local npc = Npc()	
                        if #buyableItems > 0 then
                        	local message = "I'm selling "
                        	for i = 1, #buyableItems do
