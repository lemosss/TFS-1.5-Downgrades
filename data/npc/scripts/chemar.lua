local keywordHandler = KeywordHandler:new()
 ...
 local function creatureSayCallback(cid, type, msg)
	...
	if (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'svarasd') and (not npcHandler:isFocused(cid))) then
	...
	elseif (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'svargrasdasond') and (not npcHandler:isFocused(cid))) then
		if doPlayerRemoveMoney(cid, 60) == TRUE then
		doTeleportThing(cid,{x = 32871, y = 31024, z = 4})
		npcHandler:addFocus(cid)
		else 
	...
	return true
       ...
       local travelNode = keywordHandler:addKeyword({'hills'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to fly to fermo hills for 60 gold coins?'})
		...
		local travelNode = keywordHandler:addKeyword({'xxsvasdargrond'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to fly to svargrond for 60 gold coins?'})
