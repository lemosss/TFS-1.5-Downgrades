...
function creatureSayCallback(cid, type, msg)
	...
	if not npcHandler:isFocused(cid) then
	   ...
	   elseif (msgcontains(msg, 'axe') or msgcontains(msg, 'hallowed axe') or msgcontains(msg, 'demon oak')) then
		...
		npcHandler:say('Ooh you completed the demon oak! thanks, It belongs to a Daramian nobleman named. I knew him well and he told me almost augured that someone will come who is worthy to obtain his treasure. Im sure this someone is you. Good luck in finding it!')
			talk_state = 4
			setPlayerStorageValue(cid,811355,1)
		...
		elseif msgcontains(msg, 'yes') and talk_state == 4 then
                local price = 1000
			if (getPlayerStorageValue(cid,35700) == 1) then
					npcHandler:say('Good luck, go take your reward')
					talk_state = 4
            elseif getPlayerItemCount(cid, 2386) >= 1 and getPlayerMoney(cid) >= price then
