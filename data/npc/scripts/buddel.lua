local keywordHandler = KeywordHandler:new()
 ...
 local function creatureSayCallback(cid, type, msg)
	...
	elseif (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'barbarian') and (not npcHandler:isFocused(cid))) then
		if doPlayerRemoveMoney(cid, 60) == TRUE then
		doTeleportThing(cid,{x = 32642, y = 31222, z = 7})
		npcHandler:addFocus(cid)
		else 
         selfSay('Sorry, you don\'t have enough money.') 
        end
	elseif (msgcontains(msg, 'bring') and msgcontains(msg, 'me') and msgcontains(msg, 'to') and msgcontains(msg, 'camp') and (not npcHandler:isFocused(cid))) then
		if doPlayerRemoveMoney(cid, 60) == TRUE then
		doTeleportThing(cid,{x = 32642, y = 31222, z = 7})
		npcHandler:addFocus(cid)
		else 
	...
	return true
        ...
        local travelNode = keywordHandler:addKeyword({'tyrsung'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to travel to tyrsung for 40 gold coins'})
       ...
       local travelNode = keywordHandler:addKeyword({'barbarian'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to travel to barbarian camp for 60 gold coins?'})
        	travelNode:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = false, level = 0, cost = 60, destination = {x = 32642, y = 31222, z = 7} })
        	travelNode:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, reset = true, text = 'Then stay here.'})

        local travelNode = keywordHandler:addKeyword({'camp'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to travel to barbarian camp for 60 gold coins?'})
        	travelNode:addChildKeyword({'yes'}, StdModule.travel, {npcHandler = npcHandler, premium = false, level = 0, cost = 60, destination = {x = 32642, y = 31222, z = 7} })
        	travelNode:addChildKeyword({'no'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, reset = true, text = 'Then stay here.'})
			
        keywordHandler:addKeyword({'pass'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I can take you to Tyrsung, Okolnir, Helheim and Barbarian Camp.'})
        keywordHandler:addKeyword({'travel'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I only can offer you my services to Tyrsung, Okolnir Barbarian Camp and Helheim.'})
		keywordHandler:addKeyword({'passage'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'I only can offer you my services to Tyrsung, Okolnir Barbarian Camp and Helheim.'})
