...
local function creatureSayCallback(cid, type, msg)
	...
	elseif(msgcontains(msg, "pass")) then
		npcHandler:say("You can {pass} either to the {Cemetery Quarter} or {Alchemist Quarter}. Which one will it be?", cid)
		npcHandlerfocus = 1
	elseif(msgcontains(msg, "alchemist")) then
		if(npcHandlerfocus == 1) then
			doTeleportThing(cid, {x=31279, y=31011, z=7})
			npcHandlerfocus = 0
		end
	elseif(msgcontains(msg, "cemetery")) then
		if(npcHandlerfocus == 1) then
			doTeleportThing(cid, {x=31287, y=31011, z=7})
			npcHandlerfocus = 0
		end
	end
	return true
