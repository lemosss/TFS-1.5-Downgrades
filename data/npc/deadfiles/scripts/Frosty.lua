...
local talkUser = cid
	...
	if sleighinfo[msg] ~= nil then
		...
		local text = ''
			...
			elseif #(sleighinfo[msg].items) then
				text = items_list
			elseif (sleighinfo[msg].cost > 0) and #(sleighinfo[msg].items) then
	...
	elseif msgcontains(msg, "yes") then
		if (talkState[talkUser] > 10010 and talkState[talkUser] < 11100) then
			...
			if #(sleighinfo[rtnt[talkUser]].items) > 0 then
				for i = 1, #(sleighinfo[rtnt[talkUser]].items) do
			...
			if(getPlayerMoney(cid) >= sleighinfo[rtnt[talkUser]].cost) and (items_number == #(sleighinfo[rtnt[talkUser]].items)) then
