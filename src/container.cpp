	...
	if (item->isStackable()) {
		...
		if (index == INDEX_WHEREEVER) {
			...
			for (Item* containerItem : itemlist) {
				const uint32_t stackMax = containerItem->getStackMax();
				if (containerItem != item && containerItem->equals(item) && containerItem->getItemCount() < stackMax) {
					if (queryAdd(slotIndex++, *item, count, flags) == RETURNVALUE_NOERROR) {
						n += stackMax - containerItem->getItemCount();
					}
				}
			}
		} else {
			...
			if (destItem) {
				const uint32_t stackMax = destItem->getStackMax();
				if (item->equals(destItem) && destItem->getItemCount() < stackMax) {
					if (queryAdd(index, *item, count, flags) == RETURNVALUE_NOERROR) {
						n = stackMax - destItem->getItemCount();
					}
				}
			}
		}

		maxQueryCount = freeSlots * item->getStackMax() + n;
		...
		uint32_t& flags)
{
	...
	if (autoStack && item->isStackable() && item->getParent() != this) {
		if (*destItem && (*destItem)->equals(item) && (*destItem)->getItemCount() < (*destItem)->getStackMax()) {
			return this;
		}
		...
		for (Item* listItem : itemlist) {
			if (listItem != item && listItem->equals(item) && listItem->getItemCount() < listItem->getStackMax()) {
				*destItem = listItem;
				index = n;
				return this;
			}
